...
const displayDistrictFullDetails = asyncHandler(async (req, res) => {
    const { id } = req.params;
    const district = await displayDistrictFullDetailsService(id);

    return res.status(200).json(
        new ApiResponse(200, district, "District full details fetched successfully")
    );
});
...
const display_all_Club_basedOn_user_district = asyncHandler(async (req, res) => {
    ...
    return res
        .status(200)
        .json(new ApiResponse(200, result, "Clubs in your district fetched successfully"));
});
...
const block_skater_toggle = asyncHandler(async (req, res) => {
    const clubMemberId = req.user._id;
    const { id } = req.params;
    const { block_skater_service } = await import("./club.service.js");
    const result = await block_skater_service(clubMemberId, id);
    return res.status(200).json(
        new ApiResponse(200, result, result.blocked ? "Skater blocked" : "Skater unblocked")
    );
});
export {
